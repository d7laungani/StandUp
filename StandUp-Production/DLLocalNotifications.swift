    ...
    func scheduleAllNotifications () {
        ...
        for note in queue {
            ...
            if count < min(DLNotificationScheduler.maximumScheduledNotifications, MAX_ALLOWED_NOTIFICATIONS) {
                var startTime = DateInRegion(absoluteDate:  note.fireDate!, in: region)
                print(startTime)
                self.scheduleNotification(notification: note)
                let popped = DLQueue.queue.pop()
                startTime = DateInRegion(absoluteDate:  popped.fireDate!, in: region)
                count += 1
            }
